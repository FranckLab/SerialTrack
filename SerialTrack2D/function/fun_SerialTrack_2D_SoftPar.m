...
while iterNum < maxIterNum
    ...
    if iterNum == 1 || n_neighbors > 4 % n_neighborsMax/2  % Initialize ParNotMissing
        ...
        [parNotMissingIndA,~] = find(dist_BA < max([ BeadPara.distMissing, 0.5*BeadPara.beadSize])); % Find particles not missing in Particle A
        ...
        [parNotMissingIndBCurr,~] = find(dist_AB < max([ BeadPara.distMissing, 0.5*BeadPara.beadSize]));  % Find particles not missing in Particle BCurr
