...
sigma = [1,1]; % desired standard deviation of bead [1,1]
sizeI = [4001,501]; % desired output volumetric image size.
...
[x0] = poissonDisc(sizeI,5,nBeads,1); % 5
...
nDeformations = 2; % number of deformation increments +1;   +2 for translations
...
for i = 2:nDeformations
    ...
    u{i-1}(:,2) = 2*(cos( 2*pi* (x0{1}(:,2)-251) ./  ( (300-10)/4001*(x0{1}(:,1)-1) + 10 )   ));
    u{i-1}(:,1) = 0;
    %%%%% Stretch %%%%%
...
for i = 1:length(I)
ImgCurr =  uint8(255*( I{i} + 0.0*randn(sizeI) )) ;
...
% hidden on
% trimesh(tri,x,y,z)
figure, scatter(x,y,1,z); axis equal; axis tight; colorbar;
